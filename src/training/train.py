...
from src.training.loss import (
    contrastive_loss_with_reweighting,
)  # Contrastive loss with reweighting
...
def run_training(
    ...
    *,
    ...
    # Log basic training configuration and dataset statistics
    logger.info(
        "Training config: model=%s, shared_tower=%s, batch_size=%d, epochs=%d, lr=%.2e, temperature=%.3f, "
    ...
    )  # Create optimizer (AdamW with learning rate)
    ...
    for epoch in range(epochs):  # Loop over epochs
        ...
        # Progress bar per epoch over mini-batches
        for queries, products in tqdm(
            ...
            loss = contrastive_loss_with_reweighting(
                q_emb,
                p_emb,  # Query and product embeddings
            ...
            total_loss += (
                loss.item()
            )  # Add batch loss to accumulator (detach from graph)
            n_batches += 1  # Increment batch counter
        logger.info(
            "Epoch %s/%s loss=%.4f", epoch + 1, epochs, total_loss / max(n_batches, 1)
        )
    if save_path is not None:  # If save path provided
        ...
        torch.save(
            {"model_state": model.state_dict(), "model_name": model_name}, save_path
        )
...
def main() -> int:
    ...
    p = argparse.ArgumentParser(
        description="Train two-tower encoder on ESCI"
    )  # Create argument parser
    p.add_argument(
        "--config",
        type=str,
        default="configs/train.yaml",
        help="Config YAML (optional)",
    )  # Config file path
    ...
    )  # Use shared encoder for both towers
    ...
    p.add_argument(
        "--no-reweight-hard",
        action="store_true",
        help="Disable self-adversarial reweighting",
    )
    p.add_argument("--hard-weight-power", type=float, default=None)
    p.add_argument(
        "--no-query-augment",
        action="store_true",
    ...
    p.add_argument(
        ...
        type=float,
        default=None,
        help="Prob of substituting partial query (default 0.25)",
    )
    ...
    data_dir = Path(
        args.data_dir or cfg.get("data_dir") or DATA_DIR
    )  # Use arg > config > default
    # Call training function with args/config merged (CLI args override config)
    run_training(
        data_dir=data_dir,  # Data directory
        model_name=args.model_name
        or cfg.get("model_name", "all-MiniLM-L12-v2"),  # Model name
        shared_tower=args.shared or cfg.get("shared_tower", False),  # Shared tower flag
        batch_size=args.batch_size
        if args.batch_size is not None
        else cfg.get("batch_size", 64),  # Batch size
        epochs=args.epochs
        if args.epochs is not None
        else cfg.get("epochs", 3),  # Number of epochs
        lr=args.lr if args.lr is not None else cfg.get("lr", 2e-5),  # Learning rate
        temperature=args.temperature
        if args.temperature is not None
        else cfg.get("temperature", 0.05),  # Temperature
        reweight_hard=not args.no_reweight_hard and cfg.get("reweight_hard", True),
        hard_weight_power=args.hard_weight_power
        if args.hard_weight_power is not None
        else cfg.get("hard_weight_power", 1.0),
        use_query_augment=not args.no_query_augment
        and cfg.get("use_query_augment", True),
        augment_prob=args.augment_prob
        if args.augment_prob is not None
        else cfg.get("augment_prob", 0.25),
