    ...
    *,
    ...
    if test_df is None:  # If test DataFrame not provided
        ...
        if not test_path.exists():  # If preprocessed file doesn't exist
            ...
            _, test_df = prepare_train_test(
                data_dir=base / "esci-data" / "shopping_queries_dataset"
            )
    ...
    if model is None:  # If model not provided
        model = TwoTowerEncoder(
            model_name=model_name, shared=False, normalize=True
        )  # Create new model
        if model_path is not None:  # If model checkpoint path provided
            ckpt = torch.load(
                Path(model_path), map_location=device, weights_only=True
            )  # Load checkpoint
    ...
    test_df["_row_idx"] = np.arange(
        len(test_df)
    )  # Add row index column to track original order
    scores_arr = np.full(
        len(test_df), np.nan, dtype=float
    )  # Initialize scores array with NaN
    with torch.no_grad():  # Disable gradient computation (faster, less memory)
        for _qid, grp in test_df.groupby(
            "query_id"
        ):  # Group by query_id (iterate over each query)
            query = grp["query"].iloc[
                0
            ]  # Get query string (same for all rows in group)
            products = grp[
                "product_text"
            ].tolist()  # Get list of product texts for this query
            row_indices = grp[
                "_row_idx"
            ].values  # Get original row indices for this group
            # Encode query (repeat for each product) and products
            q_embs = model.encode_queries(
                [query] * len(products), device=device
            )  # [N, D] query embeddings
            p_embs = model.encode_products(
                products, device=device
            )  # [N, D] product embeddings
            # Compute similarity: dot product (cosine for normalized embeddings)
            sim = (
                (q_embs * p_embs).sum(dim=1).cpu().numpy()
            )  # [N] similarity scores, move to CPU
            scores_arr[row_indices] = sim  # Store scores at original row positions
    test_df.drop(
        columns=["_row_idx"], inplace=True
    )  # Remove temporary row index column
    # Evaluate ranking: compute nDCG@k and MRR per query, return averages
    return evaluate_ranking(
        test_df, scores_arr, query_id_col="query_id", relevance_col="relevance", k=k
    )
...
def main() -> int:
    ...
    p = argparse.ArgumentParser(
        description="Evaluate two-tower on ESCI test set"
    ...
    metrics = run_evaluation(
        ...
        or cfg.get("model_path", "data/model.pt"),  # Model checkpoint path
        data_dir=data_dir,  # Data directory
        model_name=args.model_name
        or cfg.get("model_name", "all-MiniLM-L6-v2"),  # Model name
